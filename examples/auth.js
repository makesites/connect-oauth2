...
Auth.prototype = {
	...
	login: function( type ){
		...
		return '<html><body><button><a href="'+ url +'?client_id='+ this.options.app.client_id +'&response_type='+ type +'&redirect_uri=http://localhost:8080/auth/'+ type +'">Click to login</a></button></body></html>'
	},

	token: function( code ){
		var url = this.options.api + ( this.routes.token || this.routes.access_token );
		return url +'?grant_type=authorization_code&code='+ code +'&client_id='+ this.options.app.client_id +'&client_secret='+ this.options.app.client_secret +'&redirect_uri=http://localhost:8080/auth/token';
	},
