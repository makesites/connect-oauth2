...
Verify.prototype = {
	...
	clientID: function(req, res, next){
		...
		// process
		this.authority({ client_id: client_id }, function( err, valid ){
	...
	clientCreds: function(req, res, next){
		...
		// process
		this.authority({ client_id: client_id, client_secret: client_secret }, function( err, valid ){
			if( err ) return next( err );
			//
			if( valid ){
				next(null, true);
	...
	userCreds: function(req, res, next){
		...
		// process
		this.authority({ client_id: client_id, username: username, password: password }, function( err, valid ){
			if( err ) return next( err );
			//
			if( valid ){
				next(null, true);
