...
var middleware = function(req, res, next){
	...
	// prerequisites
	//console.log( grant_type, req.oauth );
...
OAuth2.prototype = {
	...
	getCode: function(req, res, next){
		...
		// - required fields
		var required = requiredFields( ["response_type", "client_id", "redirect_uri", "scope"], req );
		...
		var actions = [
			// validate client_id
			function(cb){
				//self.verify.clientID(req, res, function(){
					// continue...
					cb( null );
				//});
			},
			...
			function(cb){
				...
				self.store.create( code, function( result ){
					// error control?
					cp(null);
				});
		...
		var actions = [
			...
			function(cb){
				...
				self.verify.clientCreds(req, res, function( err, result ){
					if( !result ) return cp( error("unauthorized_client") );
					// continue...
					cb( null );
				});
			},
			...
			function(cb){
				...
				self.store.read({ code: code }, function( err, result ){
					// error control
					if( !result ) return cp( error("invalid_access_token"));
					var client_id = getField("client_id", req);
					if( result.meta.client_id !== client_id ){
						return cp( error("invalid_client") );
					}
					var now = (new Date()).getTime();
					var expired = parseInt( result.expires_in );
					if( expired < now ){
						return cp( error("expired_access_token") );
					}
					cp(null);
			...
			function(cb){
				//
				self.store.destroy({ code: code }, function( result ){
					// error control?
					cp(null);
				});
		...
		var actions = [
			...
			function(cb){
				...
				self.store.read({ refresh_token: refresh_token }, function( result ){
					// error control
					if( !result ) return cp( error("invalid_refresh_token") );
					...
					var now = (new Date()).getTime();
					var expired = parseInt( result.expires_in ) + (60 * 86400000);
					if( expired < now ){
						cp( error("expired_refresh_token") );
					}
					var client_id = getField("client_id", req);
					if( result.meta.client_id !== client_id ){
						cp( error("invalid_client") );
					}
					old = result;
					cp(null);
				});
